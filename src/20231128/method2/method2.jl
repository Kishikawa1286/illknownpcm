...
function  method2(A₁::Matrix{Interval{T}}, A₂::Matrix{Interval{T}})::Matrix{TwofoldInterval{T}} where {T <: Real}
    w = twofoldIntervalPCM2Weights(intervalPCM2TwofoldIntervalPCM(A₁, A₂))
    εᶿ = solveIndividualCancellingLP_m2([A₁, A₂], w.ŵᴸ, w.ŵᵁ)
...
function solveIndividualCancellingLP_m2(
    ...
    try
        ...
        optimalValue =
            sum(value.(Ŵᴸ' * εᴸ⁻)) + sum(value.(Ŵᵁ' * εᵁ⁻)) +
            sum(value.(Ŵᴸ' * εᴸ⁺)) + sum(value.(Ŵᵁ' * εᵁ⁺))
